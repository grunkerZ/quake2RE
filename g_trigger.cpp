...
THINK(fake_exit_think) (edict_t* self) -> void {
	vec3_t center = (self->absmin + self->absmax) * 0.5f;
	...
	for (size_t i = 1; i <= game.maxclients; i++) {
		...
		float dist = (ent->s.origin - center).length();

		if (dist <= self->dmg_radius) {
			if (ent->client->pers.sanity > 0) {
				ent->client->pers.sanity -= self->dmg;
				if (ent->client->pers.sanity < 0)
					ent->client->pers.sanity = 0;
			}
...
void SP_trigger_fake_exit(edict_t* self) {
	gi.setmodel(self, self->model);
	...
	if (st.distance)
		self->dmg_radius = (float)st.distance;
	else
		self->dmg_radius = 128.0f;
	if (!self->dmg)
		self->dmg = 20;

	self->think = fake_exit_think;
	self->nextthink = level.time + 100_ms;
...
void SP_info_loop_dest(edict_t* self) {
...
TOUCH(trigger_loop_touch) (edict_t* self, edict_t* other, const trace_t& tr, bool other_touching_self) ->void {
	if (!other->client)
		return;
	if (level.time < self->timestamp)
		return;
	...
	float yaw_diff = dest->s.angles[YAW] - self->ideal_yaw;

	float rad = yaw_diff * (PIf / 180.0f);
	float c = cos(rad);
	float s = sin(rad);
	...
	vec3_t new_offset;
	new_offset[0] = old_offset[0] * c - old_offset[1] * s;
	new_offset[1] = old_offset[0] * s + old_offset[1] * c;
	...
	other->s.old_origin = new_origin;
	other->client->ps.pmove.origin = new_origin;

	vec3_t old_vel = other->velocity;
	other->velocity[0] = old_vel[0] * c - old_vel[1] * s;
	...
	self->timestamp = level.time + 100_ms;
}

void SP_trigger_loop(edict_t* self) {
	

	self->ideal_yaw = self->s.angles[YAW];

	InitTrigger(self);
	self->touch = trigger_loop_touch;
	if (!self->dmg)
		self->dmg = 5;
	gi.linkentity(self);
}
