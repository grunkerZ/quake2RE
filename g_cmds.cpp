...
void Cmd_Sanity_f(edict_t* ent) {
	if (!G_CheatCheck(ent))
		return;

	if (gi.argc() < 2) {
		gi.LocClient_Print(ent, PRINT_HIGH, "Usage: sanity <value> [set/add/sub]\nCurrent Sanity: %d\n", ent->client->pers.sanity);
		return;
	}
	int val = atoi(gi.argv(1));
	const char* op = gi.argv(2);

	if (Q_strcasecmp(op, "add") == 0) {
		ent->client->pers.sanity += val;
	}
	else if (Q_strcasecmp(op, "sub") == 0) {
		ent->client->pers.sanity -= val;
	}
	else {
		ent->client->pers.sanity = val;
	...
	if (ent->client->pers.sanity > g_sanity_max->integer)
